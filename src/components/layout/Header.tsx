...
export const Header = ({ className }: HeaderProps) => {
  ...
  return (
    <>
      {/* Logo positioned outside and above the navbar */}
      <motion.div
      ...
      {/* Navbar positioned below the logo */}
      <motion.header
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        transition={{ duration: 0.8, type: "spring", stiffness: 100, delay: 0.2 }}
        className={cn(
          "fixed top-28 left-0 right-0 z-50 backdrop-blur-lg border-b border-border/50",
          className
        )}
        style={{ backgroundColor: 'rgba(5, 10, 48, 0.95)' }}
      >
        <div className="container mx-auto px-4 py-4">
          <nav className="flex items-center justify-center">
            ...
            <motion.a
              href="/contact"
              className="bg-gradient-brand text-white px-6 py-2 rounded-full shadow-brand hover:shadow-glow transition-smooth font-medium ml-8"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
